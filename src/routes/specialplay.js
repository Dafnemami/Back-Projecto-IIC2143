...
router.post('/', async (ctx) => {
  try {
    ...
    if (ctx.request.body.card === 0 || ctx.request.body.card > 12) {
    ...
    if (hand == null) {
      ...
      error.code = 400;
      throw error;
    } else if (!hand.specialhand.includes(String(ctx.request.body.card))) {
      const error = Error("You don't have that card");
      error.code = 400;
      throw error;
    }
    let scards = hand.specialhand.split(',');
    scards.splice(scards.indexOf(String(ctx.request.body.card)), 1);
    scards = scards.toString();
    hand.update({ specialhand: scards });
    await hand.save({ field: 'specialhand' });
    const play = await ctx.orm.SpecialPlay.create(ctx.request.body);
    await play.save();
    ctx.body = play;
    ctx.status = 200;
  } catch (error) {
...
router.get('/game/:id', async (ctx) => {
  try {
    ctx.body = await ctx.orm.SpecialPlay.findAll({
      where: {
